...
int read_buttons() {
 ...
 if (key_in > 440) return no_t; // Wenn alle offen sind, ist der Widerstand hoch
 if (key_in > 410) return t1;  // t1 liegt bei 426
 if (key_in > 370) return t2; // t2 liegt bei 393
 if (key_in > 310) return t3;  // t3 liegt bei 342
 return no_t;  // wenn alle anderen nicht hinhauen, ist kein Taster gedrückt (eigentlich sind aber mehrere gedrückt)
}
...
void rt_settings() {
  ...
  lcd.setCursor(0,3);
  lcd.print("--     Zurueck    ++");
  while(read_buttons()!=t2) {
    while(read_buttons()==no_t) {}
    lcd_key=read_buttons();
    if (lcd_key==t1 && global::reactTime>1.0) {
      global::reactTime--;
    } else if( lcd_key==t3 && global::reactTime<11.0) {
      global::reactTime++;
    }
    lcd.setCursor(9,2);
    lcd.print("     ");
...
void bh_settings() {
  ...
  while(read_buttons()!=t2) {
    while(read_buttons()==no_t) {}
    lcd_key=read_buttons();
    if (lcd_key==t1 && global::hbew>1) {
      global::hbew--;
    } else if( lcd_key==t3 && global::hbew<20) {
      global::hbew++;
    }
    bh=global::hbew*100;
    lcd.setCursor(11,2);
  ...
  switch (lcd_key) {
    ...
    { // Es wurde das Spiel Classic gewählt. Bevor es losgeht, muss die Anzahl der Spieler festgelegt werden.
      ...
      while(read_buttons()!=t2) {
        ...
        while (read_buttons()==no_t) {}
        lcd_key=read_buttons();
        if (lcd_key==t1 && global::players.activePlayers>1) {
          global::players.activePlayers--;
        } else if (lcd_key==t3 && global::players.activePlayers<2) {
          global::players.activePlayers++;
        } else if (lcd_key==t2) {
          break;
        }
      ...
      classic(global::players.activePlayers, global::reactTime, global::hbew); // eigentlich ist nicht nötig, die Variablen mitzugeben, weil sie global definiert sind?!
